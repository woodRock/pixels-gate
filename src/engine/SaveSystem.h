...
public:
  static void SaveGame(const std::string &filename, Registry &registry,
    ...
    if (auto *stats = registry.GetComponent<StatsComponent>(player)) {
      file << "[PLAYER_STATS]\n";
      file << stats->strength << " " << stats->dexterity << " "
           << stats->constitution << " " << stats->intelligence << " "
           << stats->wisdom << " " << stats->charisma << "\n";
      file << stats->maxHealth << " " << stats->currentHealth << " "
           << stats->damage << " " << stats->inspiration << "\n";
    ...
    if (file.fail())
      std::cerr << "Error writing quests!" << std::endl;
    ...
    for (auto &[entity, trans] : transforms) {
      ...
      if (auto *s = registry.GetComponent<StatsComponent>(entity)) {
        file << "1\n";
        file << s->maxHealth << " " << s->currentHealth << " " << s->damage
             << " " << (s->isDead ? 1 : 0) << "\n";
      } else
        file << "0\n";
      ...
      if (auto *inv = registry.GetComponent<InventoryComponent>(entity)) {
        ...
        for (const auto &item : inv->items) {
          file << item.name << "\n"
               << item.iconPath << "\n"
               << item.quantity << " " << (int)item.type << " "
               << item.statBonus << " " << item.value << "\n";
        }
      } else
        file << "0\n";
    ...
    if (file.fail())
      std::cerr << "Error writing entities!" << std::endl;
  ...
  static void
  SaveWorldFlags(const std::string &filename,
  ...
  static void LoadWorldFlags(const std::string &filename,
    ...
    while (std::getline(file, line)) {
      if (line == "[GAME_STATE]") {
        ...
        if (auto *stats = registry.GetComponent<StatsComponent>(player)) {
          ...
          file >> stats->maxHealth >> stats->currentHealth >> stats->damage >>
              stats->inspiration;
        ...
        if (auto *inv = registry.GetComponent<InventoryComponent>(player)) {
          ...
          for (int i = 0; i < count; ++i) {
            ...
            if (!(file >> std::ws && std::getline(file, name)))
              break;
            if (!(file >> std::ws && std::getline(file, icon)))
              break;

            if (!icon.empty() && std::isdigit(icon[0]) &&
        ...
        for (int i = 0; i < count; ++i) {
          ...
          if (target == INVALID_ENTITY && uniqueId.empty() &&
              genericIndex < genericEntities.size()) {
          ...
          if (target != INVALID_ENTITY) {
            ...
            if (auto *inter =
                    registry.GetComponent<InteractionComponent>(target)) {
            ...
            if (hasStats) {
              ...
              int dead;
              if (s)
                file >> s->maxHealth >> s->currentHealth >> s->damage >> dead;
              else {
                int dummy;
                file >> dummy >> dummy >> dummy >> dead;
              }
              if (s)
                s->isDead = (dead == 1);
            }
            ...
            file.ignore();
            if (invCount > 0 ||
                registry.HasComponent<InventoryComponent>(target)) {
              ...
              for (int j = 0; j < invCount; ++j) {
                ...
                file.ignore();
                if (inv)
                  inv->AddItem(iname, qty, (ItemType)type, bonus, icon, val);
              }
            }
            int hasDiag;
            file >> hasDiag;
            if (hasDiag) {
              ...
              for (int k = 0; k < nodeCount; ++k) {
                std::string nId;
                file >> std::ws;
                std::getline(file, nId);
                int optCount = 0;
                file >> optCount;
                std::vector<int> states;
                for (int m = 0; m < optCount; ++m) {
                  int s;
                  file >> s;
                  states.push_back(s);
                }
              ...
              if (auto *diag =
                      registry.GetComponent<DialogueComponent>(target)) {
                if (!diag->tree)
                  diag->tree = std::make_shared<DialogueTree>();
                ...
                for (auto &[nId, states] : nodeOptionStates) {
                  if (diag->tree->nodes.find(nId) != diag->tree->nodes.end()) {
                    ...
                    for (size_t m = 0;
                         m < states.size() && m < node.options.size(); ++m) {
            ...
            if (hasLoot) {
              ...
              if (!loot)
                loot = &registry.AddComponent(target, LootComponent{});
              loot->drops.clear();
              for (int k = 0; k < itemCount; ++k) {
                ...
                file >> qty >> type >> bonus >> val;
                file.ignore();
                loot->drops.push_back(
                    {iname, icon, qty, (ItemType)type, bonus, val});
              }
            }
          } else {
            int hasStats;
