...
namespace PixelsEngine {
    ...
    Texture::Texture(SDL_Renderer* renderer, const std::string& path)
        ...
        // so we don't strictly need SetColorKey unless we are using BMPs with a specific key.
        // For backwards compatibility with our BMP test assets, we can keep it if format is not alpha.
        // But usually, IMG_Load handles everything nicely.
    ...
    void Texture::RenderRect(int x, int y, const SDL_Rect* srcRect, int w, int h, SDL_RendererFlip flip) const {
        if (!m_Texture) return;
