...
namespace PixelsEngine {
    ...
    class Pathfinding {
    public:
        static std::vector<std::pair<int, int>> FindPath(const Tilemap& map, int startX, int startY, int endX, int endY) {
            std::vector<std::pair<int, int>> path;
            
            if (!map.IsWalkable(endX, endY)) return path; // Target unreachable

            // Open Set (Nodes to be evaluated)
            // Using a simple vector for simplicity, priority_queue is better for performance but harder to search/update
            ...
            while (!openSet.empty()) {
                ...
                for (auto it = openSet.begin(); it != openSet.end(); ++it) {
                    if ((*it)->fCost() < (*currentIt)->fCost() || 
                       ((*it)->fCost() == (*currentIt)->fCost() && (*it)->hCost < (*currentIt)->hCost)) {
                        currentIt = it;
                    }
                }
                ...
                for (int i = 0; i < 4; ++i) {
                    ...
                    if (IsInList(closedSet, nx, ny)) continue;

                    float newMovementCostToNeighbor = current->gCost + 1; // Distance between neighbors is 1
    ...
    private:
        ...
        static void RetracePath(Node* startNode, Node* endNode, std::vector<std::pair<int, int>>& path) {
